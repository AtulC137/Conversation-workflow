"""Inbound PSTN call session bootstrap via DID → workflow mapping (SIP trunk)."""
...
logger = logging.getLogger("sip_inbound")
...
def _load_did_map() -> dict[str, dict]:
    raw = os.environ.get("SIP_INBOUND_DID_MAP", "").strip()
    if not raw:
        # Fallback to FreJun map for migration convenience
    ...
    try:
        ...
        logger.error(f"[SIP INBOUND] Invalid SIP_INBOUND_DID_MAP: {e}")
        return {}
...
async def resolve_inbound_session_id(to_number: str) -> str | None:
    ...
    if not entry:
        logger.error(f"[SIP INBOUND] Unmapped DID: {did}")
        return None

    workflow_id = entry.get("workflowId") or entry.get("workflow_id")
    if not workflow_id:
        logger.error(f"[SIP INBOUND] DID {did} missing workflowId")
        return None

    user_id = (
        entry.get("userId")
        or entry.get("user_id")
        or os.environ.get("SIP_INBOUND_SYSTEM_USER_ID", "").strip()
        or os.environ.get("FREJUN_INBOUND_SYSTEM_USER_ID", "").strip()
    )
    if not user_id:
        logger.error(f"[SIP INBOUND] No userId for DID {did}")
        return None
    ...
    try:
        ...
        logger.info(f"[SIP INBOUND] Created session {session_id} for DID {did}")
        return session_id
    except Exception as e:
        logger.error(f"[SIP INBOUND] Session bootstrap failed: {e}")
        return None
