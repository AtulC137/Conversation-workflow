...
from contact_context import sanitize_tts_text
...
TTS_MODEL = "bulbul:v3"
TTS_SPEAKER = "ritu"
MIN_BUFFER_SIZE = int(os.environ.get("TTS_MIN_BUFFER_SIZE", "30"))
TTS_PACE = float(os.environ.get("TTS_PACE", "1.0"))
TTS_MAX_CHUNK_LENGTH = int(os.environ.get("TTS_MAX_CHUNK_LENGTH", "500"))
TTS_FLUSH_TIMEOUT_SEC = float(os.environ.get("TTS_FLUSH_TIMEOUT_SEC", "60"))
...
class TtsWsSession:
    ...
    def __init__(self, sample_rate: int = 16000):
        ...
        self._converted_this_turn = False
    ...
    def _configure_payload(self, target_language_code: str) -> dict:
        return {
    ...
    async def _configure_ws(self, target_language_code: str, *, force: bool = False):
        if (
            not force
            ...
            return
        payload = self._configure_payload(target_language_code)
        logger.info(f"[TTS WS] configure → {payload}")
        await self._ws.configure(**payload)
    ...
    async def configure(self, target_language_code: str):
        async with self._lock:
            await self._open()
            await self._configure_ws(target_language_code)
        ...
        async with self._lock:
            try:
                if (
                    ...
                    return
                await self._open()
                await self._configure_ws(target_language_code, force=after_abort)
    ...
    async def _safe_convert(self, text: str, *, contact_fields: dict[str, str] | None = None) -> bool:
        if self._aborted or self._ws is None or self._turn_failed:
            return False
        text = sanitize_tts_text(text, contact_fields)
        if not text or not _has_language_chars(text):
            logger.warning("[TTS WS] convert skipped — no speakable text")
            return False
        ...
        logger.info(f"[TTS WS] convert → len={len(text)} text={text[:80]!r}")
        try:
            await self._ws.convert(text)
            self._converted_this_turn = True
            return True
        except Exception as e:
    ...
    async def _safe_flush(self, timeout: float | None = None) -> bool:
        flush_timeout = TTS_FLUSH_TIMEOUT_SEC if timeout is None else timeout
        if self._aborted or self._ws is None or self._turn_failed:
            return False
        if not self._converted_this_turn:
            logger.warning("[TTS WS] flush skipped — no convert this turn")
            return False
        logger.info('[TTS WS] flush → {"type": "flush"}')
        try:
            await self._ws.flush()
            await asyncio.wait_for(self._final_event.wait(), timeout=flush_timeout)
    ...
    async def _flush_pending_buffer(self) -> bool:
        converted = False
        if self._pending_text.strip() and _has_language_chars(self._pending_text):
            converted = await self._safe_convert(self._pending_text)
        self._pending_text = ""
        return converted
    ...
    async def flush_and_wait(self, timeout: float | None = None):
        if self._aborted or self._ws is None or self._turn_failed:
            return
        async with self._lock:
            if self._aborted or self._ws is None or self._turn_failed:
            ...
            try:
                ...
                await self._safe_flush(timeout=timeout)
            finally:
                if self._receiver_task and not self._receiver_task.done():
                    self._receiver_task.cancel()
                    try:
    ...
    async def speak_full(
        ...
        prepared = sanitize_tts_text(text.strip())
        if not prepared or not _has_language_chars(prepared):
            logger.warning("[TTS WS] speak_full skipped — no speakable text")
            if done_callback:
                await done_callback()
            return
        try:
            async with self._lock:
                ...
                self._start_receiver(audio_chunk_callback)
                logger.info(
                    f"[TTS WS] speak_full [{target_language_code}]: {prepared[:80]!r}..."
                )
                if await self._safe_convert(prepared):
