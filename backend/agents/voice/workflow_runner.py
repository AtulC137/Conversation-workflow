...
# Seconds of caller silence before advancing via the no-response wire.
# Per-node `silenceTimeoutSec` is ignored — global wait-for-reply rule.
DEFAULT_SILENCE_TIMEOUT_SEC = float(os.environ.get("SILENCE_TIMEOUT_SEC", "3"))
# Brief pause between nodes when caller is not expected to speak.
NODE_GAP_SEC = float(os.environ.get("NODE_GAP_SEC", "0.35"))
...
class WorkflowRunner:
    ...
    def __init__(self, graph: dict[str, Any]):
        ...
        self.qa_answer_count: int = 0

        self.MAX_QA_ANSWERS: int = 2
    ...
    def record_qa_answer(self) -> int:

        self.qa_answer_count += 1

        return self.qa_answer_count



    def qa_limit_reached(self) -> bool:

        return self.qa_answer_count >= self.MAX_QA_ANSWERS



    def find_bye_node_id(self) -> str | None:

        for preferred in ("bye", "closing"):

            node = self.nodes.get(preferred)

            if node and node.get("type") == "conversation":
        ...
        for node in self.nodes.values():
            ...
            if title in ("bye", "closing", "goodbye"):
    ...
    def jump_to_bye(self) -> str | None:

        bye_id = self.find_bye_node_id()

        if not bye_id:

            return None

        self.current_node_id = bye_id
    ...
    def get_silence_timeout_sec(self, node_id: str | None = None) -> float:
        # Global rule: when waiting for caller reply, always ~4s then advance.
    ...
    def node_waits_for_caller(self, node_id: str | None = None) -> bool:
        ...
        if self.is_conversation_node(nid):
            # Intro is greeting-only — never wait before "are you free?"
            if nid == "intro":
                return False
            return self.get_wait_for_response(nid)
        if self.is_qa_node(nid):
            return True
        return False

    def peek_branch_target(self, branch_id: str, from_node_id: str | None = None) -> str | None:
        source = from_node_id or self.current_node_id
        if not source:
            return None
        for edge in self.edges:
