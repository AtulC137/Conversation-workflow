...
FALLBACK_EN = "Sorry, I don't have that information."
FALLBACK_HI = "क्षमा करें, मेरे पास यह जानकारी नहीं है।"
FALLBACK_HINGLISH = "Sorry, mere paas yeh information nahi hai."
...
def build_system_prompt(context: str, example: str, end_points: list[str]) -> str:
    ...
    return f"""You are a voice assistant on a phone call. Follow the persona and workflow below.
...
- Answer their question using ONLY the FACTS block above.
- If the fact exists (amount, fine, date, name), state it clearly in one sentence.
- If the fact is NOT in FACTS, say "Sorry, I don't have that information."
...
- After answering, you may briefly remind them of the current question."""
...
_NEGATIVE_ONLY = re.compile(
    ...
    r"dont\s+have\s+(?:any\s+)?questions?|nahi|nah)$",
    re.I,
)
...
RULES:
- Proactively tell the caller the requested information using ONLY the FACTS block.
- If a fact exists (amount, fine, date, name), state it clearly.
- If a requested fact is NOT in FACTS, say "Sorry, I don't have that information." for that part only.
- Do NOT ask the caller a question unless the task explicitly says to ask.
