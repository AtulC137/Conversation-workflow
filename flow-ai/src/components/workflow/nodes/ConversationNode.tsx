...
export function ConversationNode({ data, selected }: NodeProps<NodeData>) {
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border px-3.5 py-2.5">
        <div className="flex h-6 w-6 items-center justify-center rounded-md bg-muted">
          <MessageSquareText className="h-3.5 w-3.5 text-foreground" />
        </div>
        <div className="min-w-0 flex-1">
          <div className="truncate text-[13px] font-semibold leading-tight">
            {data.title?.trim() ? data.title : "Conversation"}
          </div>
      ...
      <div className="border-t border-border bg-muted/30 px-3.5 py-2.5">
        ...
        {responses.length === 0 && (
          <p className="mt-1.5 text-[10px] text-muted-foreground">No user responses</p>
        )}
      </div>
    </motion.div>
  );
